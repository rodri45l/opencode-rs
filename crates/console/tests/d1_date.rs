...
use opencode_console::date::{get_monthly_bounds, get_week_bounds, DateTime};
...
fn returns_a_monday_based_week_for_sunday_dates() {
    let (start, end) = get_week_bounds(dt(2026, 1, 18, 12, 0, 0));
...
fn returns_a_seven_day_window() {
    let (start, end) = get_week_bounds(dt(2026, 1, 14, 12, 0, 0));
    assert_eq!(start, dt(2026, 1, 12, 0, 0, 0));
    assert_eq!(end, dt(2026, 1, 19, 0, 0, 0));
}
...
fn resets_on_subscription_day_mid_month() {
    let (start, end) = get_monthly_bounds(dt(2026, 3, 20, 10, 0, 0), dt(2026, 1, 15, 8, 0, 0));
...
fn before_subscription_day_in_current_month_uses_previous_month_anchor() {
    let (start, end) = get_monthly_bounds(dt(2026, 3, 10, 10, 0, 0), dt(2026, 1, 15, 8, 0, 0));
...
fn clamps_day_for_short_months() {
    let (start, end) = get_monthly_bounds(dt(2026, 3, 1, 10, 0, 0), dt(2026, 1, 31, 12, 0, 0));
...
fn handles_subscription_on_the_first() {
    let (start, end) = get_monthly_bounds(dt(2026, 4, 15, 0, 0, 0), dt(2026, 1, 1, 0, 0, 0));
...
fn exactly_on_the_reset_boundary_uses_the_current_period() {
    let (start, end) = get_monthly_bounds(dt(2026, 3, 15, 8, 0, 0), dt(2026, 1, 15, 8, 0, 0));
    assert_eq!(start, dt(2026, 3, 15, 8, 0, 0));
    assert_eq!(end, dt(2026, 4, 15, 8, 0, 0));
}
...
fn february_to_march_with_day_30_subscription() {
    let (start, end) = get_monthly_bounds(dt(2026, 2, 15, 6, 0, 0), dt(2025, 12, 30, 6, 0, 0));
