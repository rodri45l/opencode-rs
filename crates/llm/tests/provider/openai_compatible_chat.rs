...
use opencode_llm::{providers, Auth, LLM, LLMClient, Message, ToolCallPart};
use serde_json::json;
...
fn provides_model_helpers_for_compatible_provider_families() {
    let families = [
        ("baseten", providers::openai_compatible::baseten(), "https://inference.baseten.co/v1"),
        ("cerebras", providers::openai_compatible::cerebras(), "https://api.cerebras.ai/v1"),
        ("deepinfra", providers::openai_compatible::deepinfra(), "https://api.deepinfra.com/v1/openai"),
        ("deepseek", providers::openai_compatible::deepseek(), "https://api.deepseek.com/v1"),
        ("fireworks", providers::openai_compatible::fireworks(), "https://api.fireworks.ai/inference/v1"),
        ("togetherai", providers::openai_compatible::togetherai(), "https://api.together.xyz/v1"),
    ];

    for (provider, family, base_url) in families {
        let model = family.configure(json!({ "apiKey": "test-key" })).model(&format!("{provider}-model"));
...
fn matches_ai_sdk_compatible_tool_request_body_fixture() {
    ...
    assert_eq!(prepared.body["tool_choice"], json!({ "type": "function", "function": { "name": "lookup" } }));
}
