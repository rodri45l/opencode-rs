...
fn prepares_multimodal_user_input_and_tool_history() {
    let prepared = LLMClient::prepare(LLM::request(json!({
        ...
        "messages": [
            Message::user([{ "type": "text", "text": "What is in this image?" }, { "type": "media", "mediaType": "image/png", "data": "AAECAw==" }]),
...
fn continues_image_tool_results_as_inline_vision_input_without_base64_text() {
    ...
    assert!(!prepared.body["contents"].to_string().contains("\"content\":\"AAECAw==\""));
}
...
fn omits_tools_when_tool_choice_is_none() {
    ...
    assert_eq!(prepared.body, json!({ "contents": [{ "role": "user", "parts": [{ "text": "Say hello." }] }] }));
}
...
fn preserves_thought_signature_for_reasoning_and_tool_call_continuation() {
    ...
    let reasoning_end = response.events.iter().find(|event| event["type"] == "reasoning-end").expect("reasoning-end");
    assert_eq!(reasoning_end["providerMetadata"], json!({ "google": { "thoughtSignature": "thought_sig" } }));
    let tool_call = response.events.iter().find(|event| event["type"] == "tool-call").expect("tool-call");
    assert_eq!(tool_call["providerMetadata"], json!({ "google": { "thoughtSignature": "tool_sig" } }));
}
...
fn maps_length_and_content_filter_finish_reasons() {
    ...
    assert_eq!(filtered.events.last().expect("finish")["reason"], "content-filter");
}
...
fn rejects_unsupported_assistant_media_content() {
    ...
    assert!(error.to_string().contains("Gemini assistant messages only support text, reasoning, and tool-call content for now"));
}
...
fn rejects_oversized_image_input() {
    ...
    let error = LLMClient::prepare(LLM::request(json!({
        "model": model(),
        "messages": [Message::user({ "type": "media", "mediaType": "image/png", "data": oversized })],
    })))
    .expect_err("should fail");
