...
use opencode_llm::{Auth, CacheHint, LLM, LLMClient, Message, ToolCallPart};
use serde_json::json;
...
fn passes_top_k_through_additional_model_request_fields() {
    ...
    assert_eq!(prepared.body["inferenceConfig"], json!({ "maxTokens": 64, "temperature": 0 }));
    assert_eq!(prepared.body["additionalModelRequestFields"], json!({ "top_k": 40 }));
}
...
fn decodes_text_delta_message_stop_and_metadata_usage() {
    ...
    let finishes: Vec<&serde_json::Value> = response.events.iter().filter(|event| event["type"] == "finish").collect();
...
fn assembles_streamed_tool_call_input() {
    ...
    assert_eq!(response.tool_calls, json!([{ "type": "tool-call", "id": "tool_1", "name": "lookup", "input": { "query": "weather" } }]));
    let deltas: Vec<&serde_json::Value> = response.events.iter().filter(|event| event["type"] == "tool-input-delta").collect();
    assert_eq!(
        deltas,
        vec![
    ...
    assert_eq!(response.events.last().expect("finish")["reason"], "tool-calls");
}
...
fn emits_provider_error_for_throttling_exception() {
    ...
    assert_eq!(response.events.iter().find(|event| event["type"] == "provider-error"), Some(&json!({ "type": "provider-error", "message": "Slow down", "retryable": true })));
}
...
fn classifies_input_too_long_validation_exceptions() {
    ...
    assert_eq!(
        response.events.iter().find(|event| event["type"] == "provider-error"),
        Some(&json!({ "type": "provider-error", "message": "Input is too long for requested model", "classification": "context-overflow", "retryable": false }))
    );
}
...
fn rejects_requests_with_no_auth_path() {
    ...
    let error = LLMClient::generate(LLM::update_request(base_request(), json!({ "model": unsigned })).expect("update"))
        .expect_err("should fail");

    assert!(error.to_string().contains("Bedrock Converse requires either route bearer auth or AWS credentials"));
}
...
fn emits_cache_point_markers_after_system_user_and_assistant_text() {
    ...
    let prepared = LLMClient::prepare(LLM::request(json!({
        ...
        "messages": [
            Message::user([{ "type": "text", "text": "User prefix.", "cache": cache }]),
            Message::assistant([{ "type": "text", "text": "Assistant prefix.", "cache": cache }]),
        ],
        "generation": { "maxTokens": 16, "temperature": 0 },
    })))
    .expect("prepare");

    assert_eq!(prepared.body["system"], json!([{ "text": "System prefix." }, { "cachePoint": { "type": "default" } }]));
    assert_eq!(prepared.body["messages"][0]["content"], json!([{ "text": "User prefix." }, { "cachePoint": { "type": "default" } }]));
    assert_eq!(prepared.body["messages"][1]["content"], json!([{ "text": "Assistant prefix." }, { "cachePoint": { "type": "default" } }]));
}
...
fn rejects_unsupported_image_and_document_media_types() {
    ...
    .expect_err("image");
    assert!(bad_image.to_string().contains("Bedrock Converse does not support image media type image/svg+xml"));
    ...
    .expect_err("document");
    assert!(bad_doc.to_string().contains("Bedrock Converse does not support media type application/x-tar"));
}
