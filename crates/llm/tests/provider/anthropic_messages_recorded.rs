...
use opencode_llm::{providers, LLM, LLMClient, Message, ToolCallPart};
use serde_json::json;

#[test]
#[ignore = "porting: anthropic recorded sad path not implemented"]
fn rejects_malformed_assistant_tool_order() {
    let cassette = common::recording("anthropic-messages", "rejects-malformed-assistant-tool-order-without-patch");
    assert!(cassette.get("interactions").is_some(), "cassette carries the recorded 400");
