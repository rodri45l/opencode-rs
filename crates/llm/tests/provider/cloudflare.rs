...
fn prepares_ai_gateway_models_through_the_openai_compatible_chat_protocol() {
    let model = providers::cloudflare::ai_gateway(json!({ "accountId": "test-account", "gatewayId": "test-gateway", "apiKey": "test-token" }))
    ...
    assert_eq!(prepared.body["model"], "workers-ai/@cf/meta/llama-3.3-70b-instruct");
...
fn defaults_ai_gateway_id_to_default_when_omitted_or_blank() {
    let model = providers::cloudflare::ai_gateway(json!({ "accountId": "test-account", "gatewayId": "", "gatewayApiKey": "test-token" }))
        .model("workers-ai/@cf/meta/llama-3.3-70b-instruct");

    assert_eq!(model["endpoint"]["baseURL"], "https://gateway.ai.cloudflare.com/v1/test-account/default/compat");
}
...
fn allows_a_fully_configured_base_url_override() {
    ...
    assert_eq!(prepared.model["endpoint"]["baseURL"], "https://gateway.proxy.test/v1/custom/compat");
}

#[test]
#[ignore = "porting: cloudflare provider not implemented"]
fn prepares_direct_workers_ai_models_through_the_openai_compatible_chat_protocol() {
    let model = providers::cloudflare::workers_ai(json!({ "accountId": "test-account", "apiKey": "test-token" }))
    ...
    assert_eq!(model["endpoint"]["baseURL"], "https://api.cloudflare.com/client/v4/accounts/test-account/ai/v1");
