...
use opencode_llm::{Auth, LLM, LLMClient, Message, ToolCallPart};
use serde_json::json;
...
fn replays_canonical_reasoning_as_reasoning_content() {
    ...
    assert_eq!(prepared.body["messages"], json!([{ "role": "assistant", "content": "Hello", "reasoning_content": "thinking" }]));
}
...
fn continues_image_tool_results_as_vision_input_without_base64_text() {
    ...
    assert!(!prepared.body["messages"].to_string().contains("\"content\":\"AAECAw==\""));
}
