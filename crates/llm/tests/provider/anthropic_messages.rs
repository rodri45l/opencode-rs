...
fn lowers_chronological_system_updates_natively_for_opus_48_with_cache_hints() {
    ...
    let prepared = LLMClient::prepare(LLM::request(json!({
        ...
        "messages": [
            Message::user("Before."),
            Message::system([{ "type": "text", "text": "Operator update.", "cache": CacheHint::new(json!({ "type": "ephemeral" })) }]),
...
fn lowers_image_tool_result_content_as_structured_image_blocks() {
    ...
    let block = prepared.body["messages"]
        .as_array()
        .and_then(|messages| messages.iter().find_map(|message| message["content"].as_array()))
...
fn parses_text_reasoning_and_usage_stream_fixtures() {
    ...
    assert_eq!(response.message["content"], json!([
...
fn emits_provider_error_events_for_mid_stream_provider_errors() {
    ...
    assert_eq!(response.events, json!([{ "type": "provider-error", "message": "overloaded_error: Overloaded" }]));
}
...
fn classifies_prompt_too_long_provider_errors() {
    ...
    assert_eq!(
        response.events,
        json!([{ "type": "provider-error", "message": "invalid_request_error: prompt is too long: 210000 tokens", "classification": "context-overflow" }])
    );
}
...
fn decodes_server_tool_use_and_web_search_tool_result_as_provider_executed_events() {
    ...
    assert_eq!(
        response.events.iter().find(|event| event["type"] == "tool-call"),
        Some(&json!({ "type": "tool-call", "id": "srvtoolu_abc", "name": "web_search", "input": { "query": "effect 4" }, "providerExecuted": true }))
    );
