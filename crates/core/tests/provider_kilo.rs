...
const NOTE: &str = "porting: provider kilo not implemented";
...
#[test]
#[ignore = "porting: provider kilo not implemented"]
...
fn applies_legacy_referer_headers_only_to_kilo() {
    ...
    ProviderPlugins::apply_kilo("kilo", &mut kilo).expect(NOTE);
    assert_eq!(
    ...
    ProviderPlugins::apply_kilo("openrouter", &mut openrouter).expect(NOTE);
...
fn uses_the_legacy_provider_id_guard_instead_of_endpoint_matching() {
    ...
    ProviderPlugins::apply_kilo("custom-kilo", &mut custom).expect(NOTE);
