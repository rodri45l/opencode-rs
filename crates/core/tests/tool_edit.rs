...
fn registers_with_the_locked_edit_schema() {
    let mut keys = EditTool::schema_keys().unwrap();
    keys.sort_unstable();
...
fn replaces_every_exact_occurrence_when_replace_all_is_true() {
    let outcome = EditTool::apply("same same same", "same", "after", true).unwrap();
...
fn reports_a_failure_for_the_requested_path() {
    assert_eq!(
        EditTool::failure_message("secret.txt").unwrap(),
