...
use opencode_server::message_page::{
    cursor_decode, cursor_encode, filter_compacted, page, Info, Page, PageItem, Part, Role,
    WithParts,
};
...
fn cursor_encode_decode_roundtrip() {
    let encoded = cursor_encode("msg_123", 1_234_567_890.0);
    let (id, time) = cursor_decode(&encoded);
...
fn cursor_encode_decode_with_fractional_time() {
    let encoded = cursor_encode("msg_123", 1_234_567_890.5);
    let (_, time) = cursor_decode(&encoded);
    assert_eq!(time, 1_234_567_890.5);
}

#[test]
fn encoded_cursor_is_base64url() {
    let encoded = cursor_encode("msg_123", 0.0);
...
fn page_accepts_cursors_from_fractional_timestamps() {
    ...
    let a = page(&items, 2, None);
    let b = page(&items, 2, a.cursor.as_deref());
...
fn filter_compacted_returns_all_messages_when_no_compaction() {
    ...
    let result = filter_compacted(&msgs);
