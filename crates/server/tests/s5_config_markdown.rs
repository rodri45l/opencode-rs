...
use opencode_server::config_markdown;
...
fn matches() -> Vec<(String, String)> {
    config_markdown::files(TEMPLATE)
}
...
fn should_not_match_when_preceded_by_backtick() {
    ...
    let backtick_matches = config_markdown::files(backtick_test);
...
fn should_not_match_email_addresses() {
    ...
    let email_matches = config_markdown::files(email_test);
