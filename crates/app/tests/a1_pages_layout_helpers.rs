...
//! Behaviour pinned by packages/app/src/pages/layout/helpers.ts and
//! pages/layout/deep-links.ts.
#![allow(dead_code)]

use std::collections::BTreeMap;

use opencode_app::pages_layout_helpers::{
    child_session_on_path, close_home_project, collect_new_session_deep_links,
    collect_open_project_deep_links, compare_session_time, display_name, drain_pending_deep_links,
    effective_workspace_order, error_message, has_project_permissions, home_project_directories,
    home_project_navigation, home_session_server_status, latest_root_session, parse_deep_link,
    parse_new_session_deep_link, path_key, sorted_root_sessions, toggle_home_project_selection,
    Navigation, NewSessionDeepLink, ProjectSelection, ServerStatus, Session,
};
...
fn detects_project_permissions_with_a_filter() {
    ...
    assert!(has_project_permissions(&map, |item| item == "perm-child"));
}
...
fn ignores_project_permissions_filtered_out() {
    ...
    assert!(!has_project_permissions(&map, |_| false));
}
...
fn hides_status_derived_from_an_inactive_server() {
    assert_eq!(
        home_session_server_status(false, || ServerStatus {
            working: true,
            tint: Some("red".into())
        }),
        ServerStatus {
    ...
    assert_eq!(
        home_session_server_status(true, || ServerStatus {
            working: true,
            tint: Some("red".into())
        }),
        ServerStatus {
