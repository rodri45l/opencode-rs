...
use opencode_desktop::initialization_forward::{
    await_deferred, forward_initialization_failure, make_deferred, Failure,
};

fn assert_forwarded_failure(failure: Failure) {
    let mut deferred = make_deferred();
    forward_initialization_failure(&mut deferred, failure.clone());
    let exit = await_deferred(&deferred);
    assert!(exit.is_err());
